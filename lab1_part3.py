...
for i in range(30):
    ...
    ax[i].hist(
        malignant[:, i], bins=bins, color="r", alpha=0.5
    )  # red color for malignant class
    ax[i].hist(
        benign[:, i], bins=bins, color="g", alpha=0.3
    )  # alpha is for transparency in the overlapped region
    ax[i].set_title(cancer.feature_names[i], fontsize=9)
    ax[i].axes.get_xaxis().set_visible(
        False
    )  # the x-axis coordinates are not so useful, as we just want to look how well separated the histograms are
    ax[i].set_yticks(())
...
plt.scatter(
    cancer_df["worst symmetry"],
    cancer_df["worst texture"],
    s=cancer_df["worst area"] * 0.05,
    color="magenta",
    label="check",
    alpha=0.3,
)
...
plt.scatter(
    cancer_df["mean radius"],
    cancer_df["mean concave points"],
    s=cancer_df["mean area"] * 0.05,
    color="purple",
    label="check",
    alpha=0.3,
)
...
# changle depending on the n_components
# Xax = X_pca[:, 0]
# Yax = X_pca[:, 1]
Xax = X_pca[:, 0]
Yax = X_pca[:, 2]
...
plt.colorbar()
plt.xticks(
    range(len(cancer.feature_names)), cancer.feature_names, rotation=65, ha="left"
)
plt.tight_layout()
plt.show()
